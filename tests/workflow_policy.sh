...
assert_contains .github/workflows/ci.yml "name: Run deterministic tests"
assert_contains .github/workflows/ci.yml "run: mise run test"
prek_line="$(grep -n 'j178/prek-action@' .github/workflows/ci.yml | cut -d: -f1)"
test_line="$(grep -n 'run: mise run test' .github/workflows/ci.yml | cut -d: -f1)"
[[ "$prek_line" =~ ^[0-9]+$ ]] || fail "Prek action line is missing"
[[ "$test_line" =~ ^[0-9]+$ ]] || fail "Deterministic test line is missing"
((test_line > prek_line)) || fail "Tests must run after Prek"
